...
#
#V4.2: Added a penalty for the drone going out of bounds - experiment with the cost matrix to stabilize the drone
...
sim.setObjectPosition(targetHandle, -1, [0.0, 0.0, 3.0])
# Set the drone's initial position.
sim.setObjectPosition(droneHandle, -1, [0.0, 0.0, 3.0])
...
while (t := sim.getSimulationTime()) < simualtion_time:
    ...
    targetObjPos = [
        4 * np.sin(0.9*t),  # Sine wave movement for x
        4 * np.cos(0.9*t),  # Sine wave movement for y
    ...
    if x0[6] > 0.5 or x0[6] < -0.5:
        print("Roll is out of bounds, applying penalty")
        Q[0,0] = 0.01                                   # Penalize the x position
        Q[1,1] = 0.01                                   # Penalize the y position
        Q[2,2] = 5                                      # Reward the z position
        Q[6,6] = 10                                     # Reward the roll
        Q[7,7] = 10                                     # Reward the pitch                                   
        R[1,1] = 0.001
        R[2,2] = 0.001
        stabilise_mode = True
        x_st = x0[0]
        y_st = x0[1]                                   
    elif x0[7] > 0.5 or x0[7] < -0.5:
        print("Pitch is out of bounds, applying penalty")
        Q[0,0] = 0.01
        Q[1,1] = 0.01
        Q[2,2] = 5
        Q[6,6] = 10
    ...
    if stabilise_mode == True: # If the drone is out of bounds, stabilize it
        ref = np.array([x_st, y_st, targetPos[2],
                    0, 0, 0,
                    0, 0, 0,
                    0, 0, 0])
    else:
