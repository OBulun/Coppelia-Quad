...
sim.setObjectPosition(targetHandle, -1, [0.0, 2.0, 3.0])
# Set the drone's initial position.
sim.setObjectPosition(droneHandle, -1, [0.0, 2.0, 3.0])
...
while (t := sim.getSimulationTime()) < simualtion_time:
    ...
    """
    targetObjPos = [
        2 * np.sin(0.4*t),  # Sine wave movement for x
        2 * np.cos(0.4*t),  # Sine wave movement for y
        3.0             # Fixed altitude (z)
    ]
    sim.setObjectPosition(targetHandle, -1, targetObjPos)
    """
    targe_lin_vel, _ = sim.getObjectVelocity(targetHandle)
    ...
    ref = np.array([targetPos[0], targetPos[1], targetPos[2],
                    targe_lin_vel[0], targe_lin_vel[1], targe_lin_vel[2],
                    0, 0, 0,
                    0, 0, 0])
